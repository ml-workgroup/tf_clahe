...
{   
    ...
    for(int x0 = 0; x0 < width - windowWidth +1; ++x0){
        ...
        for(int i = 0; i < clipPos; ++i){
            cdfSum += min(float(histogram[i]), absoluteClipLimit);
        }
        float cdfAtPosition = cdfSum + min(float(histogram[clipPos]), absoluteClipLimit) / 2;
...
int main()
{
    ...
    */
    ...
    // Compute CLAHE
    ClaheKernel<<<size - windowSize + 1, size - windowSize + 1>>>(
        data, out,
        lookUpTable, uniqueValues, nUniqueValues,
        size, size, size, windowSize, windowSize, windowSize, 
        0.1, true);
    
    // Wait for GPU to finish before accessing on host and check for errors
